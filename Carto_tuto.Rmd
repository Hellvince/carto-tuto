...
Le slot @data Nos données spatiales sont donc correctement importées. On peut dès à présent fabriquer une carte très simple, avec comme fond le contours des communes de l'Ile-de-France (région numéro 11). On peut très bien le faire à l'échelle de la France entière, mais vu le nombre élevé de communes, la production de cette carte risque de prendre un certain temps (vous êtes prévenus !).

A titre d'exemple, on utilise ici la nouvelle méthode de `plot`, spécifique à la classe `SpatialPolygons` et ajouté par le package `sp` :
```{r}
